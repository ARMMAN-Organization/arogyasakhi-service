...
3. **Hypotension — "shock features."** Confirmed: use the existing clinical signs/symptoms already captured on the ANC/High-Risk assessment (the shared danger-signs multiselect) rather than adding a new question. **Implemented** — dizziness, breathlessness, and palpitation (from that same multiselect) are treated as shock features; Hypotension now grades Mild on the systolic-BP threshold (<90 mmHg) OR any of these three signs being present, matching the "Systolic <90 mmHg or associated with shock features" wording in §1.3.
