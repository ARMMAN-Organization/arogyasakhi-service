...
describe('EscalationRepository', () => {
  ...
  describe('findManyByIds', () => {
    it('queries with an IN(...) filter over the given ids, excluding soft-deleted rows', async () => {
      findMany.mockResolvedValue([]);

      await repository.findManyByIds(['event-1', 'event-2']);

      expect(findMany).toHaveBeenCalledWith({
        where: { id: { in: ['event-1', 'event-2'] }, isDeleted: false },
      });
    });
  });
});
