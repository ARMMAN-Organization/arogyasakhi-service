 ...
 * Sakhi `userId`s — kept as a plain string here (not `.transform()`ed)
 * because `createDocumentedRouter()` cannot introspect a ZodEffects as an
 * OpenAPI parameter object; the split into an array happens in the
 * controller. Mirrors beneficiary-service's by-ids-with-risk-query.dto.ts.
 */
