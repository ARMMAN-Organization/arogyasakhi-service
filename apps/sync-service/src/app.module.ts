...
import { buildSyncServiceOpenApiDocument } from './docs/openapi';
...
export function createApp(prisma: PrismaService): Application {
  ...
  // Built from syncBatchModule.registry — every route registered via
  // createDocumentedRouter() above is already in the spec, so this can never
  // drift from what's actually mounted.
  api.use(createSwaggerRouter(buildSyncServiceOpenApiDocument(syncBatchModule.registry)));
  api.use(syncBatchModule.router);
  app.use('/api/v1', api);
