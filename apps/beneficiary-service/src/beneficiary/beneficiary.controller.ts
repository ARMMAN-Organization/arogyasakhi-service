...
import type { listFullDetailQuerySchema } from './dto/list-full-detail.dto';
...
export function createBeneficiaryController(service: BeneficiaryService) {
  return {
    ...
    listFullDetail: asyncHandler(async (req, res, next) => {
      if (!req.user) return next(unauthorized());
      const authorizationHeader = req.header('authorization');
      if (!authorizationHeader) return next(unauthorized());
      const query = req.query as unknown as z.infer<typeof listFullDetailQuerySchema>;
      res.json(ok(await service.listFullDetail(query, req.user, authorizationHeader)));
    }),
