/**
 * Backfill logic for the SRS "Unique ID" field (State(2)-District(3)-
 * Block(3)-ID(6), see beneficiary.unique-id.ts). Shared by:
 * - the post-migrate hook (../migrations/20260907000000_add_beneficiary_unique_id/post-migrate.ts),
 *   run automatically by `npm run migrate` via tools/prisma-foreach.js
 * - backfill-unique-id.ts, a thin standalone CLI entrypoint for running it manually
 *
 * Must run AFTER migration 20260907000000_add_beneficiary_unique_id
 * (nullable column + sequence) and BEFORE migration
 * 20260907000100_enforce_beneficiary_unique_id_not_null (which requires
 * every row to already have a value) — the post-migrate hook convention
 * guarantees this ordering automatically; a manual run must respect it too.
 *
 ...
 * runs as a service account (POST /auth/service-token) since there is no
 * human caller's bearer token to forward.
 *
...
/**
 * Runs the backfill. Resolves once complete; sets no process.exitCode
 * itself — the caller (hook or CLI entrypoint) decides how to surface
 * `failed > 0`, since a hook run must abort the migration sequence on
 * failure while other callers might want different behavior.
 */
export async function backfillUniqueId(): Promise<{ processed: number; failed: number }> {
  ...
  try {
    ...
    // state), but at the point this runs, migration 20260907000000 has only
    // added it as nullable. Raw SQL sidesteps that type/runtime mismatch for
    // this one query.
