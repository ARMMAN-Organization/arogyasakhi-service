...
/**
 * True if `existing` was generated by a different rule version than the one
 * that just ran. localScheduleUuid is deterministic on (beneficiaryId,
 * visitCode) alone — not on the evaluating rule version — so a rule pack
 * republish naturally revisits the same uuid; that's not a corrupt retry,
 * it's the same schedule slot re-evaluated under a newer, currently-published
 * rule version.
 */
function isFromDifferentRuleVersion(
  generatedByRuleVersionId: string,
  existing: { generatedByRuleVersionId: string },
): boolean {
  return existing.generatedByRuleVersionId !== generatedByRuleVersionId;
}
    ...
    for (const row of schedules) {
      ...
      if (existing) {
        if (!matchesStoredRow(row, existing)) {
          if (isFromDifferentRuleVersion(generatedByRuleVersionId, existing)) {
            throw conflict(
              `localScheduleUuid "${row.localScheduleUuid}" is already stored with different ` +
                `content under rule version "${existing.generatedByRuleVersionId}" — regenerating ` +
                'an existing schedule under a new rule version is not supported; supersede it ' +
                'explicitly instead.',
            );
          }
          throw conflict(
        ...
        if (isFromDifferentRuleVersion(generatedByRuleVersionId, existing)) {
          await this.repository.updateGeneratedByRuleVersionId(
            existing.id,
