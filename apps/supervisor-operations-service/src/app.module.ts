...
import { createOperationsModule } from './operations/operations.module';
import { buildSupervisorOperationsOpenApiDocument } from './docs/openapi';
...
export function createApp(prisma: PrismaService): Application {
  ...
  const operationsModule = createOperationsModule(prisma);

  // All routes live under the global `api/v1` prefix.
  const api = express.Router();
  api.use(createHealthRouter(prisma));
  api.use(createSwaggerRouter(buildSupervisorOperationsOpenApiDocument(operationsModule.registry)));
  api.use(operationsModule.router);
  app.use('/api/v1', api);
