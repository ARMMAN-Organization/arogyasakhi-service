...
import { VisitSummaryClient } from './visitSummary.client';
...
import { registerVisitSummaryRoutes } from './visitSummary.routes';
 ...
 *
 * All 6 sub-domains (call logs, events, inventory, gatherings, training
 * topics, visit summary) register into this single shared `doc` —
 * `createDocumentedRouter()` produces exactly one OpenAPI registry per call,
 * so splitting into independently-created registries here would silently
 * produce 6 incomplete Swagger docs instead of 1 complete one.
 */
export function createOperationsModule(prisma: PrismaService): DocumentedRouter {
  ...
  const visitSummaryClient = new VisitSummaryClient();
  const service = new OperationsService(repository, sakhiClient, visitSummaryClient);
  ...
  registerVisitSummaryRoutes(doc, service);
  return doc;
}
