...
describe('ReopenRequestRepository', () => {
  const findMany = jest.fn();
  const prisma = { reopenRequest: { findMany } } as never;
  let repository: ReopenRequestRepository;
  ...
  beforeEach(() => {
    jest.clearAllMocks();
    repository = new ReopenRequestRepository(prisma);
  });
  ...
  describe('findManyPaginated', () => {
    ...
    it('returns a nextCursor and trims the extra row when more results exist beyond the limit', async () => {
      ...
      expect(result.items).toHaveLength(1);
      expect(result.items[0].id).toBe('reopen-1');
    ...
    it('decodes a supplied cursor into a createdAt/id keyset filter', async () => {
      ...
      const cursor = Buffer.from(
        JSON.stringify({ createdAt: '2026-08-05T00:00:00.000Z', id: 'reopen-5' }),
      ).toString('base64url');
      ...
      expect(call.where.OR).toEqual([
        ...
        { createdAt: new Date('2026-08-05T00:00:00.000Z'), id: { lt: 'reopen-5' } },
      ]);
    });
