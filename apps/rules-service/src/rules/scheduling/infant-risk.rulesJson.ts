 ...
 *  3. Respiratory distress still has no confirmed HR-visit-trigger value —
 *     HR-visit trigger remains false pending ARMMAN's answer (Cord
 *     infection above is resolved; this one is not). Neuro-developmental
 *     status IS now resolved (see the separate #191 list below) — it no
 *     longer shares this open item.
 ...
 *     both sheets).
 *
 * Separately, issue #191's later 7-item ANC/infant clarification round
 * (docs/Appendix_D_High_Risk_Detection_Rules.md §1.8) answered:
 *  5. Neuro-developmental status: all milestones achieved -> Normal, any
 *     missed -> Severe (was Mild) with an HR-visit trigger (was none).
 *  6. Activity Level (new condition, ACTIVITY_LEVEL): active_and_moving ->
 *     Normal, reduced_movement -> Mild (assessment finding, no referral),
 *     lethargic -> Severe + referral.
 *  6. Feeding Concerns: FEEDING_ADEQUACY now reads the dedicated
 *     feeding_concerns multiselect (any value other than the
 *     mutually-exclusive no_concerns grades High Risk + referral), not
 *     current_feeding_practice as before.
 *  6. Infant Deformity: confirmed NOT to create a grading threshold without
 *    further clinical confirmation — still not graded by this pack.
 *  7. Prematurity: confirmed it must NOT affect grading on NN/INC/CCV
 *    visits, only remain a birth/registration record — matches this pack's
 *    existing behavior (no prematurity field is read here) unchanged.
 *  - Sickle Cell: still ANC/registration-scoped only (see
 *    anc-risk.rulesJson.ts's own note) — not read by this pack.
 *
...
const handler = (input) => {
  ...
  // not asked on NEONATAL_VISIT). Issue #191 item 1 (answered): the Sakhi
  // checks the child against an age-appropriate milestone checklist
  // (external module, not modeled as separate fields here) — all achieved
  // grades Normal; any missed milestone grades Severe and triggers an HR
  // visit (previously graded Mild with no HR trigger). ---
  ...
  if (typeof milestonesOnTrack === 'string') {
    const grade = milestonesOnTrack === 'yes' ? 'NORMAL' : 'SEVERE';
  ...
  // --- Activity Level (INFANT_VISIT/INC_VISIT/CCV_VISIT and NEONATAL_VISIT
  // both use activity_level) — new condition, issue #191 item 6 (answered):
  // active & moving -> Normal; reduced movement -> captured as an
  // assessment finding (Mild, no referral); lethargic -> High Risk,
  // referral (Severe). ---
  const activityLevel = input.activity_level;
  if (typeof activityLevel === 'string') {
    let activityGrade = 'NORMAL';
    if (activityLevel === 'reduced_movement') activityGrade = 'MILD';
    else if (activityLevel === 'lethargic') activityGrade = 'SEVERE';
    record('ACTIVITY_LEVEL', activityGrade, { activityLevel }, {
      referralTrigger: activityGrade === 'SEVERE',
      hrVisitTrigger: false,
    });
  }
  ...
  // --- Feeding adequacy. Issue #191 item 6 (answered): grade from the
  // dedicated feeding_concerns multiselect (INFANT_VISIT/INC_VISIT/
  // CCV_VISIT/NEONATAL_VISIT), not current_feeding_practice — any concern
  // other than the mutually-exclusive 'no_concerns' value grades High Risk
  // (referral + health message). Replaces this condition's prior
  // current_feeding_practice/'not_able_to_drink_feed' derivation. ---
  const feedingConcerns = input.feeding_concerns;
  const feedingConcernCodes = Array.isArray(feedingConcerns) ? feedingConcerns : [feedingConcerns];
  const hasFeedingConcern =
    feedingConcernCodes.length > 0 && feedingConcernCodes.indexOf('no_concerns') === -1;
  if (typeof feedingConcerns === 'string' || Array.isArray(feedingConcerns)) {
    const grade = hasFeedingConcern ? 'MILD' : 'NORMAL';
    record('FEEDING_ADEQUACY', grade, { hasFeedingConcern }, {
      referralTrigger: hasFeedingConcern,
      hrVisitTrigger: false,
    });
  }
