 ...
 * consistent with the rule-set CRUD in ruleSet.routes.ts.
 */
export function registerRuleVersionRoutes(doc: DocumentedRouter, service: RuleVersionService) {
  const controller = createRuleVersionController(service);
