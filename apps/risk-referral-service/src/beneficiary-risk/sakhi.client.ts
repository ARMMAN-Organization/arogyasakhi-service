 ...
 * the gateway. Used to scope `GET /beneficiaries/:beneficiaryId/risk` to a
 * Supervisor's own roster — duplicated from `referrals/sakhi.client.ts`
 * rather than imported, per the forklift rule (no cross-feature imports
 * within a service either, only shared framework code).
 */
