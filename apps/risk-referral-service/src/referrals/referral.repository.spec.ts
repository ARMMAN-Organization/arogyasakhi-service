...
describe('ReferralRepository — findManyPaginated', () => {
  const findMany = jest.fn();
  const prisma = { referral: { findMany } } as never;
  let repository: ReferralRepository;
  ...
  it('returns a nextCursor and trims the extra row when more results exist beyond the limit', async () => {
    ...
    expect(result.items).toHaveLength(1);
    expect(result.items[0].id).toBe('referral-1');
  ...
  it('decodes a supplied cursor into a createdAt/id keyset filter', async () => {
    ...
    const cursor = Buffer.from(
      JSON.stringify({ createdAt: '2026-08-05T00:00:00.000Z', id: 'referral-5' }),
    ).toString('base64url');
    ...
    expect(call.where.OR).toEqual([
      ...
      { createdAt: new Date('2026-08-05T00:00:00.000Z'), id: { lt: 'referral-5' } },
    ]);
  });
